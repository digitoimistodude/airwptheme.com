...
} // end airwptheme_move_jquery_into_footer
...
function enqueue_theme_scripts() {
  ...
  wp_localize_script( 'scripts', 'airwptheme_screenReaderText', array(
  ...
  wp_localize_script(
    'scripts',
    'airwptheme_externalLinkDomains',
    [
      'localhost:3000',
      'airdev.test',
      'dudetest.xyz',
...
function enqueue_polyfills() {
  ...
  if ( file_exists( get_theme_file_path( get_asset_file( 'legacy.js' ) ) ) ) {
    wp_register_script( 'airwptheme_legacy', '', [], filemtime( get_theme_file_path( get_asset_file( 'legacy.js' ) ) ), false );
    wp_enqueue_script( 'airwptheme_legacy' );
    wp_add_inline_script( 'airwptheme_legacy', $script, true );
  }
}
